...
export default function AddExpenseModal({ group, currentUserId, onClose, onSaved, existingExpense }) {
  const activeMembers = group.members.filter((m) => m.status === 'active' && m.userId)
  ...
  const [paidBy, setPaidBy] = useState(existingExpense?.paidBy ?? currentUserId)
  ...
  const [manual, setManual] = useState(() => {
    if (!existingExpense?.splits || existingExpense.splitMode !== 'manual') return {}
    const splitMap = Object.fromEntries(existingExpense.splits.map((s) => [s.userId, s.amount]))
    const out = {}
  ...
  function switchToManual() {
    ...
    if (validAmount && sharedIds.length > 0) {
      const arr = equalSplitAmounts(numericAmount, sharedIds.length)
      setManual(Object.fromEntries(sharedIds.map((id, i) => [id, arr[i].toFixed(2)])))
    } else {
      setManual({})
    }
    setSplitMode('manual')
  ...
  function switchToEqual() {
    ...
    if (hasInput && !confirm('Discard custom amounts and split equally?')) return
    setManual({})
    setSplitMode('equal')
    setError('')
  }
  ...
  function handleSubmit(e) {
    ...
    if (!sharedIds.length) return setError('Pick at least one person to split with.')
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-[480px] rounded-2xl bg-surface p-6"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="mb-4 flex items-center justify-between border-b border-line pb-4">
          <h2 className="text-lg font-semibold text-ink">{existingExpense ? 'Edit expense' : 'Add expense'}</h2>
          <button
            onClick={onClose}
          ...
          <div>
            ...
            {splitMode === 'equal' && validAmount && sharedIds.length > 0 && (
              <p className="mt-2 text-xs text-ink-muted">
                Split equally · {formatCurrency(numericAmount / sharedIds.length)} each
              </p>
            )}
            {splitMode === 'manual' && validAmount && sharedIds.length > 0 && (
              <div className="mt-2 flex items-center justify-between text-xs">
                <span className={manualMatches ? 'text-pos' : 'text-ink-muted'}>
                  Assigned {formatCurrency(manualSum)} of {formatCurrency(numericAmount)}
                  {!manualMatches && (
