...
// Optional `settlements` array: each { paidBy, paidTo, amount } adjusts balances
// so that settled debts no longer appear as outstanding.
export function calculateNetBalances(expenses, members, settlements = []) {
  const balances = {};
  ...
  // Apply settlements: paidBy gets +amount (debt reduced), paidTo gets -amount
  // (receivable reduced). Both converge toward zero.
...
export function userBalanceInGroup(
  expenses,
  members,
  userId,
  settlements = [],
) {
  const balances = calculateNetBalances(expenses, members, settlements);
